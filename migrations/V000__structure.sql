...
CREATE TABLE IF NOT EXISTS wallet.user_wallet (
...
CREATE TABLE IF NOT EXISTS wallet.transfer (
    ...
    CONSTRAINT transfer_payer_FK FOREIGN KEY (user_payer_id) REFERENCES wallet.`user`(user_id),
    CONSTRAINT transfer_payee_FK FOREIGN KEY (user_payee_id) REFERENCES wallet.`user`(user_id)
)
ENGINE=InnoDB
DEFAULT CHARSET=utf8
COLLATE=utf8_general_ci;
